...
export class SSHConnectionManager {
   ...
   */
  public setConfig(
    cfg: GlobalConfig | SshConnectionConfigMap,
    defaultName?: string,
  ): void {
    this.disconnect();
    ...
    if (cfg && typeof cfg === "object" && "projects" in cfg) {
      ...
      for (const [flatName, entry] of flat) {
        ...
        configs[flatName] = c as SSHConfig;
      }
      globalAuditStore.retentionDays = (global as any).audit?.retentionDays;
    } else {
      this.currentConfig = null;
      configs = cfg as SshConnectionConfigMap;
    }
  ...
  private auditName(name?: string): string {
    return name || this.defaultName;
  }

  private resolveFlatName(name?: string): string {
    if (!name) {
    ...
    if (this.currentConfig) {
      ...
      if (result) {
        if (result.ambiguous) {
          throw new ToolError(
            "AMBIGUOUS_HOST",
            `Ambiguous host '${name}': candidates ${result.candidates.join(", ")}`,
            false,
          );
        }
  ...
  private async listDirectoryInternal(
    ...
    if (this.getTransportMode(config) === "shell") {
      throw new ToolError(
        "UNSUPPORTED_IN_SHELL_MODE",
        "Current bastion shell mode does not support SFTP directory listing. Use execute-command with ls instead.",
        false,
      );
    }
    ...
    try {
      entries = await this.withTimeout(
        new Promise<any[]>((resolve, reject) => {
          sftp.readdir(validatedRemotePath, (err: Error | undefined, list: any[]) =>
            err ? reject(err) : resolve(list),
          );
        }),
        sftpTimeoutMs,
        () => this.invalidateConnection(key),
        `SFTP readdir timed out after ${sftpTimeoutMs}ms`,
      );
    } catch (error) {
